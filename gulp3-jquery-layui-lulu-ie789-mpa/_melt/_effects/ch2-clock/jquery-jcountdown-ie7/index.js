jQuery(function () {
    ...
    var g = function (b) {
        var c = b.children(".panel"), b = b.children(".countdown"), d = c.find(".timeText").val(),
            f = parseFloat(c.find(".timeZone").val()), a = c.find(".style[checked]").val(),
            l = c.find(".color[checked]").val(),
            m = "auto" == c.find(".widthType[checked]").val() ? 0 : c.find(".width").slider("value"),
            n = c.find(".textGroupSpace").slider("value"), e = c.find(".textSpace").slider("value"),
            g = "checked" == c.find(".reflection").attr("checked"), h = c.find(".reflectionOpacity").slider("value"),
            i = c.find(".reflectionBlur").slider("value"), j = c.find(".dayTextNumber").slider("value"),
    ...
    jQuery(".page").each(function (b, c) {
        var d = jQuery(c), f = d.children(".countdown"), a = d.children(".panel"), d = j[b];
        f.jCountdown(d);
        ...
        a.find(".width").slider({min: 50, max: 900, step: 1, value: f.children().width()});
        a.find(".textGroupSpace").slider({min: 0, max: 100, step: 1, value: d.textGroupSpace});
        a.find(".textSpace").slider({min: 0, max: 50, step: 1, value: d.textSpace});
        a.find(".reflection").attr("checked", d.reflection);
        a.find(".reflectionOpacity").slider({min: 0, max: 100, step: 1, value: d.reflectionOpacity});
        d.reflection ? a.find(".reflectionOpacity").parents(".group").show() : a.find(".reflectionOpacity").parents(".group").hide();
        a.find(".reflectionBlur").slider({min: 0, max: 10, step: 1, value: d.reflectionBlur});
        d.reflection ? a.find(".reflectionBlur").parents(".group").show() : a.find(".reflectionBlur").parents(".group").hide();
        a.find(".dayTextNumber").slider({min: 2, max: 8, step: 1, value: d.dayTextNumber});
        ...
        a.find("input[type=radio]").click(function () {
            var a = jQuery(this), c = a.parents(".page");
        ...
        a.find(".slider").slider({start: f, slide: f, stop: f});
        ...
        a.find(".start").click(function () {
            var c = jQuery(this), b = c.parents(".page").children(".countdown");
        ...
        a.find(".destroy").click(function () {
            var a = jQuery(this), c = a.parents(".page"), b = c.children(".panel"), c = c.children(".countdown");
        ...
        a.find(".create").click(function () {
            var a = jQuery(this), c = a.parents(".page"), b = c.children(".panel");
    ...
    jQuery("[tip]").mouseenter(function () {
        var b = jQuery(this), c = jQuery("#tip"), d = b.attr("tip"), e = b.innerWidth() - 10,
        ...
        var b = jQuery(this), c = jQuery("#tip");
        b.attr("tip");
        b = b.offset().top - c.outerHeight() - 5;
        c.stop(!0, !0).css("opacity", "1").css("top", b);
        c.animate({opacity: 0, top: "+=3"}, 150, function () {
            jQuery(this).hide()
        })
    })
});