    ...
    <header class="myHeader">
        ...
        <ul class="menu">
            <li v-for="link in links">
                ...
                <!-- 新开窗口 -->
                <a v-if="(!!loginUserName || link.href.indexOf('/'+visitUserName+'#!/console')===-1) && link.newPage"
                ...
                <!-- 未登录并且访问链接是博客设置页面 -->
                <a v-if="link.href.indexOf('/'+visitUserName+'#!/console')!==-1 && !loginUserName"
                   @click="preIntercept(link.href, link.newPage)">
                    {{link.name}}
                </a>
            </li>
        </ul>
