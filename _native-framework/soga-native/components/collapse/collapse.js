+function () {
    ...
    var accordionCallapse = {
        ...
        open: function (item) {
            ...
            U.addClass(item, 'accordion-item-expand');
        },
        close: function (item) {
            var content = item.querySelector('.accordion-item-content');
            U.removeClass(item, 'accordion-item-expand');
            setTransitionDuration(content, 0);
            content.style.height = content.scrollHeight + 'px';
            var clientLeft = content.clientLeft;
            setTransitionDuration(content, '');
    ...
    for (var i = 0; i < collapse.length; i++) {
        addEvent(collapse[i].querySelector('.accordion-item-toggle'), 'click', function () {
            var parent = U.parent(this, 'accordion-list');
            if (U.hasClass(parent, 'accordion-list')) {
            ...
            if (!con) return;
            if (U.hasClass(this.parentNode, 'accordion-item-expand')) {
