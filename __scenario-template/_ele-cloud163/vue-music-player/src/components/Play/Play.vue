    ...
    <div class="content">
      ...
      <transition name="fade">
        <div v-show="isShowMusicList" class="music-list">
          <ul>
            <li @click="toggleMusic(index)" v-for="(item, index) of musicData"
                :class="{ activeColor: index === audio.index}">{{ (index + 1) +'. &nbsp;'+ item.name}}
            </li>
          </ul>
        </div>
      </transition>

      <img @click.stop.prevent="isShowSkinColors=false;isShowMusicList=false"
           :src="audio.musicImgSrc || (musicData[0]&&musicData[0].musicImgSrc) || defaultImg" alt="microzz.com">
      ...
      <div class="progress">
        ...
        <div @click="changeTime($event)" @touchmove="touchMove($event)" @touchend="touchEnd($event)" ref="progressBar"
             class="progress-bar">
